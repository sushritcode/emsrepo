...
if (isset($strUserId) && isset($strPassword) && isset($strPlanId) && isset($strQuantity))
{
    ...
    if (is_array($arrAuthUserResult) && sizeof($arrAuthUserResult) > 0)
    {
        ...
        $DBuser_id = $arrUserDetails[0]['user_id'];
        $email_address = $arrUserDetails[0]['email_address'];
        $UserTimeZone = $arrUserDetails[0]['timezones'];
        $UserGMT = $arrUserDetails[0]['gmt'];

        $user_id = explode("usr", $DBuser_id);
        $user_id = ltrim($user_id[1], "0");
        $order_id = "ord" . time() . $user_id . strlen($user_id);
        ...
        try
        {
            $addOrderMaster = insOrderMaster($DBuser_id, $email_address, $order_id, $payment_gateway_name, $payment_from, $local_datetime, $gmt_datetime, $ip_address, $objDataHelper);
        }
        catch (Exception $a)
        {
        ...
        if ($strORM_Status == 1)
        {
            ...
            try
            {
                ...
                $total_amount = ($plan_cost * $strQuantity);
                $service_tax_amount = ($total_amount * $service_tax_percent) / 100;
                $amount = $total_amount - $service_tax_amount;
                $conversion_rate = '0.00';
                $addOrderDetails = insOrderDetails($strORM_Order_Id, $plan_id, $plan_name, $currency, $plan_cost, $strQuantity, $amount, $service_tax_percent, $service_tax_amount, $total_amount, $conversion_rate, $objDataHelper);
            }
            catch (Exception $a)
            {
