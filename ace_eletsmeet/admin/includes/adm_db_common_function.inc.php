...
function getClientList($dataHelper)
{
    ...
    try
    {
        $strSqlQuery = "SELECT pd.partner_name, cld.client_id, cld.partner_id, cld.client_name, cld.client_creation_dtm, cld.client_login_enabled FROM client_login_details AS cld, partner_details AS pd WHERE cld.partner_id = pd.partner_id AND cld.client_login_enabled = '1' ORDER BY pd.partner_name, cld.client_name";
        $arrResult = $dataHelper->fetchRecords("QR", $strSqlQuery);
        return $arrResult;
    }
    catch (Exception $e)
...
function getClientDtlsById( $client_id, $dataHelper)
{
    ...
    try
    {
        $strSqlQuery = "SELECT  * FROM  client_login_details WHERE client_id ='".trim($client_id)."'";
        $arrResult = $dataHelper->fetchRecords("QR", $strSqlQuery);
        return $arrResult;
    }
    catch (Exception $e)
