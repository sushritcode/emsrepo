...
Class CError
{
    ...
    function RaiseError($ErrorFile, $ErrorCode, $strErrMsg, $blnShowError)
    {
        ...
        //$strTrace = "Error Occured in File : ".$ErrorFile." | Error No: ".$strErrorNo." | Error Message : ".$strErrMsg."\n";
    ...
    function ErrorHandler($errno, $errmsg)
    {
        ...
        switch ($errno)
        {
            ...
            default:
                ...
                if (ADM_MAIL_ERROR_LOG)
                {
                    error_log(date("l dS of F Y h:i:s A")." ".$trace."<BR>".$strDetails, 1, ADM_ERROR_LOG_EMAIL, "Subject: Error reporting - ".$ADMIN_SITE_ROOT."\nFrom:Error Handler\nContent-type: text/html; charset=iso-8859-1\n");
                }
                if (ADM_LOG_ERROR)
                {
                    if (ADM_ERROR_LOG_FILE_NAME == "")
                    {
                        error_log(date("l dS of F Y h:i:s A")." ".$errmsg.$strDetails, 0);
                    }
                    else
                    {
                        error_log(date("l dS of F Y h:i:s A")." ,".$errmsg."\n", 3, ADM_LOGS_PATH.ADM_ERROR_LOG_FILE_NAME);
                    }
                }
                $action = $ADMIN_SITE_ROOT."error/";
