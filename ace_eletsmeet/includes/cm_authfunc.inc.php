...
function isAuthenticUser($email_address, $password, $dataHelper) {
	...
	try {
		$strSqlStatement = "SELECT lu.user_id, lu.client_id, lu.partner_id, lu.email_address, ud.nick_name FROM login_users lu , user_details ud WHERE lu.email_address='" . trim($email_address) . "' AND lu.password='" . trim($password) . "' AND  lu.login_enabled = '1' and lu.user_id = ud.user_id";
...
function updUserLastLoginDtls($email_address, $user_id, $datetime, $ipaddress, $dataHelper) {
	...
	try {
		print $strSqlStatement = "UPDATE login_users SET user_lastlogin_dtm = '" . trim($datetime) . "', user_login_ip_address = '" . trim($ipaddress) . "' WHERE user_name = '" . trim($email_address) . "' AND user_id='" . trim($user_id) . "';";
...
function getUserDetailsByID($email_address, $dataHelper) {
	...
	try {
		...
		$strSqlStatement = "SELECT lu.user_id, lu.user_name , nick_name , first_name , last_name , country_name , timezones , gmt , phone_number , idd_code , mobile_number "
			. "FROM user_details AS ud, login_users AS lu, client_details AS cd "
