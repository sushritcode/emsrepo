...
try
{
    if (isset($_POST['pwd_submit']))
    {
        ...
        if (sizeof($errors) == 0)
        {
            try
            {
                $arrUpdPwdResult = updateUserPassword($strCk_email_address, MD5($old_password), MD5($new_password), $objDataHelper);
            }
            catch (Exception $a)
            {
            ...
            $strDB_User_Id   = trim($arrUpdPwdResult[0]['@USER_ID']);
            $strDB_Client_Id = trim($arrUpdPwdResult[0]['@CLIENT_ID']);
    ...
    <div class="container">
        ...
        <div class="row">
				...
				<form name="frmChangePassword" method="POST" action="<?php echo $_SERVER["PHP_SELF"]; ?>">
                                        ...
                                        <div><input type="password" name="txtOldPassword" placeholder="Current Password" maxlength="15" class="span3" id="nickname"></div>
					<div class="frm-fields tBold">New Password<span class="required">&nbsp;*</span><span class="colon">:&nbsp;&nbsp;</span></div>
					<div><input type="password" name="txtNewPassword" placeholder="New Password" maxlength="15" class="span3" id=""></div>
					<div class="frm-fields tBold">Confirm New Password<span class="required">&nbsp;*</span><span class="colon">:&nbsp;&nbsp;</span></div>
					<div><input type="password" name="txtCnfPassword" placeholder="Confirm New Password" maxlength="15" class="span3" id=""></div></br>
