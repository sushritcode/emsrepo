...
require_once(INCLUDES_PATH.'common.inc.php');
...
//        echo "<pre/>";
        ...
        if ((is_array($arrSchDtls)) && (sizeof($arrSchDtls)) > 0)
        {
            ...
            //$CREATE_MEETING_API_URL = $Meeting_Instance.VIDEO_SERVER_API.VIDEO_SERVER_CREATE_API;
            //$JOIN_MEETING_API_URL = $Meeting_Instance.VIDEO_SERVER_API.VIDEO_SERVER_JOIN_API;
                ...
                else if (($SG_Time <= $Current_GMT_Datetime) && ($EG_Time >= $Current_GMT_Datetime))
                {
                    ...
                    if ($Schedule_Status == 0)
                    {
                        ...
                        if ($CMAPI_ReturnCode == "SUCCESS")
                        {
                            ...
                            foreach ($arrInviteesList as $aILKey => $aILVal)
                            {
                                $numbers .= "+".$aILVal["invitee_idd_code"].$aILVal["invitee_mobile_number"].",";
                            }
                            $numbers = explode(",", substr_replace($numbers, "", -1));

                            $smsMeetingTitle = implode(" ", array_splice(explode(" ", $Meeting_Title), 0, 10));

                            if (count(explode(" ", $Meeting_Title)) > 10)
                            {
                                $smsMeetingTitle = $smsMeetingTitle."...";
                            }
                            else if (strlen($smsMeetingTitle) > 70)
                            {
                                $smsMeetingTitle = substr($smsMeetingTitle, 0, 70)."...";
                            }

                            $msg = "Alert: Meeting ".$smsMeetingTitle.", called by ".$User_NickName." has started.";
                            ...
                            {
                                throw new Exception("Error in sendSMS.".$a->getMessage(), 4103);
                            }
