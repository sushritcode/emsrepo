...
try
{
   //session_start();
    ...
    try
    {
        $arrContactGroupList = getContactGroupList($strCK_user_id, $objDataHelper);
    }
    catch (Exception $a)
    {
    ...
    if (isset($_POST['btnSubmit']))
    {
        ...
        if (strlen($strEmail) != 0)
        {
            try
            {
                $isEmailExists = isContactEmailExists($strEmail, $strCK_user_id, $strCk_client_id, $objDataHelper);
        ...
        if (strlen($strGroupNameTxt) != 0)
        {
            try
            {
                $isGroupExists = isContactGroupExists($strGroupNameTxt, $strCK_user_id, $strCk_client_id, $objDataHelper);
        ...
        if (sizeof($errors) == 0)
        {
            try
            {
                ...
                $addContactDetails = insContactDetails($strNickName, $strFirstName, $strLastName, $strEmail, $strIddCode, $strMobile, $strGroupName, PERSONAL_CONTACT_TYPE, $strCK_user_id, $objDataHelper);
                if ($addContactDetails[0]['@STATUS'] == 1)
                {
                    //$_SESSION['txtErrorMsg'] = 'Contact <b><font color=#006699>"'.$strNickName.'" </font></b>added successfully.';
                    $msg = 'Contact <b><font color=#006699>"'.$strNickName.'" </font></b>added successfully.';
                    header("Location:".$SITE_ROOT.'contacts/?msg='.urlencode($msg));
                }
                else
                {
