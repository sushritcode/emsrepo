...
function IsClientNameExists($client_name, $dataHelper)
{
    ...
    try
    {
        $strSqlQuery = "SELECT COUNT(client_name) FROM client_details WHERE client_name = '" . trim($client_name) . "' AND status = '1'";
...
function getClientId($dataHelper)
{
    ...
    try
    {
        $strSqlStatement = "SELECT MAX(client_id) FROM client_details";
...
function getClientListbyName($partner_name, $client_name, $dataHelper)
{
    ...
    try
    {
        $strSqlQuery = "SELECT pd.partner_name, cd.client_id, cd.client_name, cd.client_creation_dtm, cd.status FROM client_details AS cd, partner_details AS pd WHERE cd.partner_id = pd.partner_id AND pd.partner_name LIKE '%" . trim($partner_name) . "%' AND cd.client_name LIKE '%" . trim($client_name) . "%' AND cd.status = '1' ORDER BY pd.partner_name, cd.client_name";
        $arrResult = $dataHelper->fetchRecords("QR", $strSqlQuery);
        return $arrResult;
    }
    catch (Exception $e)
