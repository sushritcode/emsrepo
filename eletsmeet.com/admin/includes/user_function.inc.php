...
function getUserList($dataHelper)
{
    ...
    try
    {
        //$strSqlQuery = "SELECT pd.partner_name, cd.client_name, ud.user_id,ud.client_id, ud.email_address, ud.nick_name, ud.first_name, ud.last_name, ud.country_name, ud.timezones, ud.gmt, ud.phone_number, ud.idd_code, ud.mobile_number, ud.registration_dtm, ud.is_admin, ud.status FROM user_details AS ud, client_details AS cd , partner_details AS pd WHERE pd.partner_id = ud.partner_id AND cd.client_id = ud.client_id AND cd.status = '1' ORDER BY pd.partner_name, cd.client_name, ud.nick_name, ud.first_name, ud.last_name, ud.email_address";
        $strSqlQuery = "SELECT pd.partner_name, cd.client_name, ud.user_id,ud.client_id, ud.email_address, ud.nick_name, ud.first_name, ud.last_name, ud.country_name, ud.timezones, ud.gmt, ud.phone_number, ud.idd_code, ud.mobile_number, ud.registration_dtm, ud.is_admin, ud.status FROM user_details AS ud, client_details AS cd , partner_details AS pd WHERE pd.partner_id = ud.partner_id AND cd.client_id = ud.client_id ORDER BY pd.partner_name, cd.client_name, ud.nick_name, ud.first_name, ud.last_name, ud.email_address";
