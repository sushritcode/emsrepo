    ...
    function ErrorHandler($errno, $errmsg)
    {
        ...
        switch ($errno)
        {
            ...
            default:
                ...
                if (CLIENT_MAIL_ERROR_LOG)
                {
                    error_log(date("l dS of F Y h:i:s A")." ".$trace."<BR>".$strDetails, 1, CLIENT_ERROR_LOG_EMAIL, "Subject: Error reporting - ".$CLIENT_SITE_ROOT."\nFrom:Error Handler\nContent-type: text/html; charset=iso-8859-1\n");
                }
                
                if (CLIENT_LOG_ERROR)
                {
                    if (CLIENT_ERROR_LOG_FILE_NAME == "")
                    {
                        error_log(date("l dS of F Y h:i:s A")." ".$errmsg.$strDetails, 0);
                    }
                    else
                    {
                        error_log(date("l dS of F Y h:i:s A")." ,".$errmsg."\n", 3, CLIENT_LOGS_PATH.CLIENT_ERROR_LOG_FILE_NAME);
                    }
                }
                
                echo $action = $CLIENT_SITE_ROOT."error/";
