...
$CLIENT_CONST_PAGEID = 'Change Password';
...
try
{
    if (isset($_POST['pwd_submit']))
    {
        ...
        if (sizeof($errors) == 0)
        {
            try
            {
                $arrUpdPwdResult = updateClientPassword($strCk_email_address, MD5($old_password), MD5($new_password), $objDataHelper);
            }
            catch (Exception $a)
            {
    ...
    {
        header("Location:" . $CLIENT_SITE_ROOT . 'profile');
    }
}
catch (Exception $e)
{
