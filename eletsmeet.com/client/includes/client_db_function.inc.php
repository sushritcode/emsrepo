...
/* -------------------------------------------------------------------------------
  Function Name : getUserDetailsByClient
...
function getUserDetailsByClient($partner_id, $client_id, $dataHelper) {
    if (!is_object($dataHelper))
    {
        throw new Exception("client_db_function.inc.php : getUserDetailsByClientName : DataHelper Object did not instantiate", 104);
    }

    try
    {
        $strSqlQuery = "SELECT ud.user_id, ud.client_id, cd.client_name, ud.partner_id, pd.partner_name, ud.email_address, "
                . "ud.nick_name, ud.first_name, ud.last_name, ud.country_name, ud.timezones, ud.gmt, "
                . "ud.phone_number, ud.idd_code, ud.mobile_number, ud.registration_dtm, ud.is_admin, ud.status "
                . "FROM user_details AS ud, client_details AS cd , partner_details AS pd "
                . "WHERE pd.partner_id = ud.partner_id AND cd.client_id = ud.client_id "
                . "AND ud.partner_id =  '" . trim($partner_id) . "' AND ud.client_id =  '" . trim($client_id) . "' AND cd.status = '1' "
                . "ORDER BY ud.first_name, ud.last_name, ud.email_address;";
        $arrResult = $dataHelper->fetchRecords("QR", $strSqlQuery);
        return $arrResult;
    }
    catch (Exception $e)
    {
        throw new Exception("client_db_function.inc.php : Error in getUserDetailsByClientName." . $e->getMessage(), 734);
    }
}
...
function updateUserStatus($user_id, $status, $dataHelper) {
    if (!is_object($dataHelper))
    {
...
function getTotalConsumedLicenseByClientId($client_id, $dataHelper) {
    ...
    try
    {
        //$strSqlQuery = "SELECT COUNT(*) AS ConsumedLicense FROM user_details WHERE STATUS ='1' AND client_id = '" . trim($client_id) . "';";
        $strSqlQuery = "SELECT COUNT(*) AS ConsumedLicense FROM user_details WHERE client_id = '" . trim($client_id) . "';";
        $arrResult = $dataHelper->fetchRecords("QR", $strSqlQuery);
        return $arrResult;
    }
    catch (Exception $e)
...
function getUnUsedPlanByClientId($client_id, $dataHelper) {
    ...
    try
    {
        $strSqlQuery = "SELECT * FROM client_subscription_master WHERE client_id='" . trim($client_id) . "' AND order_id NOT IN (SELECT order_id FROM  subscription_master);";
        $arrResult = $dataHelper->fetchRecords("QR", $strSqlQuery);
        return $arrResult;
    }
    catch (Exception $e)
...
function getMeetingCountByUser($client_id, $dataHelper) {
    if (!is_object($dataHelper))
    {
        throw new Exception("client_db_function.inc.php : getMeetingCountByUser    : DataHelper Object did not instantiate", 104);
    }
    ...
    try
    {
        $strSqlQuery = "SELECT pd.partner_id,cd.client_id,cd.client_name,ud.first_name,ud.last_name,ud.user_id,COUNT(ud.user_id) "
                ...
                . "FROM partner_details AS pd, client_details AS cd, user_details AS ud, schedule_details AS sd "
                . "WHERE pd.partner_id = cd.partner_id AND cd.client_id = ud.client_id AND ud.user_id = sd.user_id AND cd.client_id = '" . trim($client_id) . "' "
    ...
    {
        throw new Exception("client_db_function.inc.php : Error in getMeetingCountByUser." . $e->getMessage(), 734);
    }
}
